...
def prepare_image(im_path, title):
    # Abre a imagem do poster
    ...
    while font.getsize(title)[0] > (w-10):
        font_size -= 2
        font = ImageFont.truetype(font_path, font_size)

    # Desenha o título no poster em verde
    draw.text((5, 10), title, (0, 255, 0), font=font)
...
def put_poster_in_camera(poster_img, poster_shape, background_img, background_shape, homography_matrix):
    # Define as dimensões do poster e a partir delas cria uma matriz com os cantos respectivos
    poster_h, poster_w, _ = poster_shape
    poster_corners = np.float32([[0, 0], [0, poster_h - 1],
                                 [poster_w - 1, poster_h - 1], [poster_w - 1, 0]]).reshape(-1, 1, 2)

    # Converte as coordenadas do poster para coordenadas da câmera
    camera_poster_corners = cv2.perspectiveTransform(poster_corners, homography_matrix)

    # Define a região do poster na câmera e preenche com a imagem do poster
    background_h, background_w, _ = background_shape
    warped_poster = cv2.warpPerspective(poster_img, homography_matrix, (background_w, background_h))

    # Cria uma array de zeros com as mesmas dimensões da câmera
    mask = np.zeros(background_shape, dtype=np.uint8)

    # Define uma cor neutra para a máscara de acordo com
    # o número de canais na câmera e preenche a região do poster
    mask_color = (255,) * background_shape[2]
    cv2.fillConvexPoly(mask, np.int32(camera_poster_corners), mask_color)

    # Inverte a seleção da máscara para a região externa ao poster
    # e preenche esse espaço com a imagem da câmera
    mask = cv2.bitwise_not(mask)
    masked_image = mask    # Substituir por [cv2.bitwise_and(background_img, mask)] para inserir a imagem da câmera

    # Mescla o poster com a câmera, fazendo com que ele apareça no espaço correto
    return cv2.bitwise_or(warped_poster, masked_image)
...
def augment_posters(posters, calib_mtx, calib_dst):
    ...
    poster_images = []
    for p in posters:
        poster_images.append(prepare_image(p['src'], p['title']))
    ...
    while True:
        ...
        poster_image = poster_images[posters_good_matches.index(best_matches)]
        ...
        if len(best_matches) > MIN_MATCHES:
            ...
            try:
                ...
                # Insere imagem do poster com título na câmera
                img2 = put_poster_in_camera(poster_image, poster_image.shape, img2, img2.shape, M)
